...
def _exp_decay(t, h0, T):
    return h0 * np.exp(-t / T)
...
def compute_recession_analysis(
    ...
    # Compute daily derivative
    diff = np.diff(values)

    # Find recession segments (consecutive negative derivative)
    segments = []
    in_recession = False
    start_idx = 0
    ...
    # Handle segment ending at series end
    if in_recession:
        end_idx = len(diff)
        duration = end_idx - start_idx
        drop = values[start_idx] - values[end_idx]
    ...
    for start_idx, end_idx in segments:
        ...
        h0_init = seg_values[0]

        try:
            popt, _ = curve_fit(_exp_decay, t, seg_values, p0=[h0_init, 50.0],
                                bounds=([0, 1], [np.inf, 10000]), maxfev=5000)
            h0_fit, T_fit = popt
            predicted = _exp_decay(t, h0_fit, T_fit)
