...
export function RecessionPanel({ data }: Props) {
  if (!data?.segments?.length) return <p className="text-xs text-text-muted">No recession segments detected</p>
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-3">
        {[
          { label: 'T median', value: data.T_median_days != null ? `${data.T_median_days}d` : '—' },
          { label: 'Segments', value: data.n_segments },
          { label: 'T std', value: data.T_std_days != null ? `±${data.T_std_days}d` : '—' },
        ].map(kpi => (
      ...
      <div className="grid grid-cols-2 gap-3">
        {data.mrc && (
          <Plot
            ...
            layout={{ ...layout, xaxis: { ...layout.xaxis, title: { text: 'Days' } }, yaxis: { ...layout.yaxis, title: { text: 'Normalized level' } } }}
        ...
        <Plot
          ...
          layout={{ ...layout, xaxis: { ...layout.xaxis, title: { text: 'T (days)' } }, yaxis: { ...layout.yaxis, title: { text: 'Count' } },
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead><tr className="text-text-muted border-b border-white/5">
            <th className="text-left px-2 py-1.5">Start</th><th className="text-left px-2 py-1.5">End</th>
            <th className="text-right px-2 py-1.5">T (days)</th><th className="text-right px-2 py-1.5">R²</th>
          ...
          <tbody>{data.segments.map((s: any) => (
            <tr key={s.start} className="border-b border-white/5 hover:bg-bg-hover">
              ...
              <td className="px-2 py-1 text-right text-text-muted">{s.duration_days}d</td>
            </tr>
          ))}</tbody>
        </table>
      </div>
