  ...
  const criteria: CriterionProps[] = [
    {
      ...
      pass: stowa.evp_pass,
      detail: stowa.evp_pass ? t('cleanup.stowa.variancePass') : t('cleanup.stowa.varianceFail'),
      tooltip: t('cleanup.stowa.varianceTooltip'),
    },
    {
      label: t('cleanup.stowa.autocorrelation'),
      pass: stowa.autocorrelation_pass,
      detail: stowa.runs_test_pvalue != null ? `p = ${stowa.runs_test_pvalue.toFixed(3)}` : t('cleanup.stowa.notEvaluated'),
      tooltip: t('cleanup.stowa.autocorrelationTooltip'),
    },
    {
      label: stowa.t95_days != null ? `t95 = ${t('cleanup.stowa.t95Days', { value: stowa.t95_days.toFixed(0) })}` : 't95',
      pass: stowa.t95_pass,
      detail: stowa.t95_threshold != null ? t('cleanup.stowa.t95Threshold', { value: stowa.t95_threshold.toFixed(0) }) : t('cleanup.stowa.notEvaluated'),
      tooltip: t('cleanup.stowa.t95Tooltip'),
    },
    {
      label: t('cleanup.stowa.gain'),
      pass: stowa.gain_pass,
      detail: stowa.gain_significance != null ? t('cleanup.stowa.gainSignificance', { value: stowa.gain_significance.toFixed(2) }) : t('cleanup.stowa.notEvaluated'),
      tooltip: t('cleanup.stowa.gainTooltip'),
    },
  ]
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-stretch gap-2">
        ...
        <div
          title={t('cleanup.stowa.verdictTooltip')}
        ...
        >
          {stowa.overall_pass === null ? (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-medium border border-white/10 text-text-muted">
              {t('cleanup.stowa.partial')}
            </span>
          ) : (
            <>
              <span className="text-base leading-none">{stowa.overall_pass ? '✓' : '!'}</span>
              <span className="text-xs text-center">
                {stowa.overall_pass ? t('cleanup.stowa.accepted') : t('cleanup.stowa.warningRequired')}
              </span>
            </>
          )}
        </div>
