...
export function RegionalResidualsPanel({ data }: Props) {
  ...
  if (!data?.index?.length) return <p className="text-xs text-text-muted">{t('pastas.regional.notEnough')}</p>
  ...
  const traces: any[] = data.siblings?.map((s: any, i: number) => ({
    ...
    showlegend: i === 0, ...(i === 0 ? { name: t('pastas.regional.neighbors') } : {}),
  })) ?? []

  traces.push({
    x: data.index, y: data.model_residual_zscore, type: 'scatter' as const, mode: 'lines' as const,
    name: t('pastas.regional.modelResiduals'), line: { color: '#22d3ee', width: 2 },
  })
    ...
    <div>
      <Plot
        ...
        layout={{ ...darkLayout, margin: { t: 10, r: 20, b: 40, l: 55 }, height: 250,
          yaxis: { ...darkLayout.yaxis, title: { text: t('pastas.regional.zScore') } },
      ...
      {data.correlation_with_siblings != null && (
        <p className="text-[10px] text-text-muted mt-1">
          {t('pastas.regional.avgCorr')} <span className="font-mono text-text-secondary">{data.correlation_with_siblings}</span>
          {data.correlation_with_siblings > 0.5 ? <span className="text-blue-400">{t('pastas.regional.regionalPattern')}</span> : <span className="text-yellow-400">{t('pastas.regional.stationSpecific')}</span>}
        </p>
      )}
    </div>
  )
