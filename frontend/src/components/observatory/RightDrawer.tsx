...
const ZONE_LAYERS: { id: string; label: string; group: string; color?: string }[] = [
  { id: 'regions', label: 'Regions', group: 'Administrative' },
  { id: 'depts', label: 'Departments', group: 'Administrative' },
  { id: 'bassins', label: 'Basins (SANDRE)', group: 'Administrative' },
  { id: 'her', label: 'Hydroecoregions (HER-2)', group: 'Administrative', color: '#34d399' },
  { id: 'region-hydro', label: 'Hydrographic regions', group: 'SANDRE Zoning', color: '#3b82f6' },
  { id: 'secteur-hydro', label: 'Hydrographic sectors', group: 'SANDRE Zoning', color: '#06b6d4' },
  { id: 'sous-secteur-hydro', label: 'Hydro sub-sectors', group: 'SANDRE Zoning', color: '#14b8a6' },
  { id: 'zone-hydro', label: 'Hydrographic zones', group: 'SANDRE Zoning', color: '#10b981' },
]

const OVERLAY_LAYERS: { id: string; label: string; group: string; color: string }[] = [
  { id: 'cours-eau-1', label: 'Major rivers (>100km)', group: 'Hydrographic network', color: '#60a5fa' },
  { id: 'cours-eau-2', label: 'Secondary rivers', group: 'Hydrographic network', color: '#93c5fd' },
  { id: 'plan-eau', label: 'Lakes', group: 'Hydrographic network', color: '#38bdf8' },
  { id: 'masse-eau-riv', label: 'Water bodies (WFD)', group: 'Hydro-ecology', color: '#c084fc' },
]
...
export function RightDrawer(props: Props) {
  ...
  return (
    <>
      <button onClick={() => setDrawerOpen(v => !v)} aria-label={drawerOpen ? 'Close panel' : 'Open panel'} className={`absolute top-4 right-4 z-20 p-2.5 rounded-lg border transition-colors ${drawerOpen ? 'bg-accent-cyan/20 border-accent-cyan/30 text-accent-cyan' : 'bg-bg-card/90 backdrop-blur-md border-white/10 text-text-secondary hover:text-text-primary'}`}>
      ...
      <div className={`absolute top-0 right-0 h-full z-30 w-full sm:w-80 bg-bg-card border-l border-white/10 shadow-2xl transition-transform duration-200 ease-out overflow-y-auto ${drawerOpen ? 'translate-x-0' : 'translate-x-full pointer-events-none'}`}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          <h2 className="text-sm font-semibold text-text-primary">Control Panel</h2>
          <button onClick={() => setDrawerOpen(false)} className="p-1 hover:bg-bg-hover rounded" aria-label="Close"><X className="w-4 h-4 text-text-secondary" /></button>
        </div>

        <AccordionSection id="data" title="Data" defaultOpen>
          <div className="flex gap-2">
            <button onClick={() => props.setShowPiezo(!props.showPiezo)} aria-pressed={props.showPiezo} className={`flex-1 px-3 py-2 rounded-lg text-xs font-medium transition-colors border ${props.showPiezo ? 'bg-accent-cyan/20 text-accent-cyan border-accent-cyan/30' : 'bg-bg-primary text-text-secondary border-white/10'}`}>Piezometry</button>
            <button onClick={() => props.setShowHydro(!props.showHydro)} aria-pressed={props.showHydro} className={`flex-1 px-3 py-2 rounded-lg text-xs font-medium transition-colors border ${props.showHydro ? 'bg-accent-indigo/20 text-accent-indigo border-accent-indigo/30' : 'bg-bg-primary text-text-secondary border-white/10'}`}>Hydrometry</button>
            <div className="flex flex-col gap-1 mt-2">
              <label className="flex items-center gap-2 cursor-pointer group"><input type="checkbox" checked={props.showExcluded} onChange={() => props.setShowExcluded(!props.showExcluded)} className="w-3.5 h-3.5 accent-gray-500 rounded" /><span className="text-xs text-text-secondary group-hover:text-text-primary transition-colors">Filtered stations <span className="text-text-secondary/60">(grey)</span></span></label>
              <label className="flex items-center gap-2 cursor-pointer group"><input type="checkbox" checked={props.showTerrain} onChange={() => props.setShowTerrain(!props.showTerrain)} className="w-3.5 h-3.5 accent-amber-600 rounded" /><span className="text-xs text-text-secondary group-hover:text-text-primary transition-colors">Terrain <span className="text-text-secondary/60">(hillshade)</span></span></label>
            </div>
          </div>
        </AccordionSection>

        <AccordionSection id="filters" title="Filters" badge={(() => {
          const parts: string[] = []
          if (props.showPiezo && props.filteredPiezo != null && props.totalPiezo != null) parts.push(props.filteredPiezo !== props.totalPiezo ? `${props.filteredPiezo.toLocaleString()}/${props.totalPiezo.toLocaleString()} piezo` : `${props.totalPiezo.toLocaleString()} piezo`)
          ...
          <div className="space-y-3">
            ...
            <div><label className="text-xs text-text-secondary block mb-1">Min observations (days)</label><input type="number" value={props.filters.minObservations ?? ''} onChange={(e) => props.setFilter('min_obs', e.target.value || undefined)} placeholder="500" className="w-full px-2.5 py-1.5 bg-bg-primary border border-white/10 rounded text-sm text-text-primary focus:outline-none focus:border-accent-cyan/50" /></div>
            <div><label className="text-xs text-text-secondary block mb-1">Last measurement after</label><input type="date" value={props.filters.lastMeasurementAfter ?? ''} onChange={(e) => props.setFilter('last_after', e.target.value || undefined)} className="w-full px-2.5 py-1.5 bg-bg-primary border border-white/10 rounded text-sm text-text-primary focus:outline-none focus:border-accent-cyan/50" /></div>
            {props.hasSpatialFilter && (<div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-accent-cyan/10 border border-accent-cyan/20 text-accent-cyan text-xs"><span>Active spatial filter (selected area)</span><button onClick={() => props.onResetSpatial?.()} className="ml-auto hover:text-white transition-colors" aria-label="Remove spatial filter"><X className="w-3 h-3" /></button></div>)}
            {hasActiveFilter && (<button onClick={resetFilters} className="flex items-center gap-1.5 w-full justify-center px-3 py-2 rounded-lg text-xs font-medium text-text-secondary hover:text-text-primary bg-white/5 hover:bg-white/10 border border-white/10 transition-colors"><RotateCcw className="w-3 h-3" />Reset</button>)}
          </div>
        </AccordionSection>

        <AccordionSection id="layers" title="Layers">
