...
function formatPeriod(period: string, lang: string): string {
  ...
  const names = lang.startsWith('en') ? MONTH_NAMES_EN : MONTH_NAMES
  return `${names[parseInt(month, 10) - 1]} ${year}`
}

type Season = 'winter' | 'spring' | 'summer' | 'autumn'

function getSeasonLabel(period: string, t: TFunction): string {
  const month = parseInt(period.split('-')[1], 10)
  if (month <= 2 || month === 12) return t('observatory.timeline.winter')
  if (month <= 5) return t('observatory.timeline.spring')
  if (month <= 8) return t('observatory.timeline.summer')
  return t('observatory.timeline.autumn')
}
...
export function TimelineSlider({
  ...
  const SEASONS: { id: Season; label: string; months: number[] }[] = [
    ...
    { id: 'spring', label: t('observatory.timeline.spring'), months: [3, 4, 5] },
    { id: 'summer', label: t('observatory.timeline.summer'), months: [6, 7, 8] },
    { id: 'autumn', label: t('observatory.timeline.autumn'), months: [9, 10, 11] },
  ]
    ...
    <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-10 w-[calc(100%-2rem)] max-w-4xl bg-bg-card/95 backdrop-blur-md border border-white/10 rounded-lg px-4 py-3 shadow-xl">
      <div className="flex items-center gap-3 mb-2 flex-wrap">
        <div className="flex items-center gap-1.5 text-[11px]">
          <span className="text-text-secondary">{t('observatory.timeline.from')}</span>
          ...
          <span className="text-text-secondary">{t('observatory.timeline.to')}</span>
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <button onClick={() => jumpTo(0)} className="w-7 h-7 flex items-center justify-center rounded-full text-text-secondary hover:text-text-primary hover:bg-white/10 transition-colors" title={t('observatory.timeline.start')}><ChevronFirst className="w-4 h-4" /></button>
          ...
          <button onClick={() => jumpTo(maxSliderPos)} className="w-7 h-7 flex items-center justify-center rounded-full text-text-secondary hover:text-text-primary hover:bg-white/10 transition-colors" title={t('observatory.timeline.end')}><ChevronLast className="w-4 h-4" /></button>
          {currentPeriod && (<div className="text-sm ml-1"><span className="text-text-primary font-medium">{formatPeriod(currentPeriod, i18n.language)}</span><span className="text-text-secondary ml-2">- {getSeasonLabel(currentPeriod, t)}</span></div>)}
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleDeactivate} className="p-1.5 rounded hover:bg-bg-hover text-text-secondary hover:text-text-primary transition-colors" title={t('observatory.timeline.closeTimeline')}><X className="w-4 h-4" /></button>
        </div>
      </div>
