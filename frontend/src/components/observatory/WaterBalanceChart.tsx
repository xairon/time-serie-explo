...
export function WaterBalanceChart({ data }: Props) {
  ...
  const COMPONENTS = [
    { key: 'wb_effective_precip', label: t('observatory.pastas.effectivePrecip'), color: '#22c55e' },
    { key: 'wb_recharge', label: t('observatory.pastas.recharge'), color: '#3b82f6' },
    { key: 'wb_actual_evaporation', label: t('observatory.pastas.evaporation'), color: '#ef4444' },
    { key: 'wb_surface_runoff', label: t('observatory.pastas.runoff'), color: '#f97316' },
  ] as const
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-text-primary">{t('observatory.pastas.waterBalance')}</h3>
      ...
      <ResponsiveContainer width="100%" height={260}>
        <AreaChart data={chartData} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
          ...
          <Tooltip contentStyle={CHART_TOOLTIP_STYLE} labelFormatter={(v: any) => new Date(v).toLocaleDateString(localeTag, { year: 'numeric', month: 'long' })} formatter={(value: any, name: any) => { const comp = COMPONENTS.find((c: any) => c.key === name); return [value != null ? `${Number(value).toFixed(1)} mm` : '--', comp?.label ?? name] }} />
