...
import { era5PointsToSquares, era5AnomalyPointsToSquares, era5GenericAnomalyToSquares, ERA5_CELL_HALF } from './era5-grid'
...
describe('era5GenericAnomalyToSquares', () => {
  it('builds one polygon per non-null point, centred ±ERA5_CELL_HALF with a closed ring of 5 coords', () => {
    const fc = era5GenericAnomalyToSquares([
      { latitude: 48, longitude: 2, anomaly: 1.5 },
    ])
    expect(fc.type).toBe('FeatureCollection')
    expect(fc.features).toHaveLength(1)
    const f = fc.features[0]
  ...
  it('carries anomaly in feature properties', () => {
    const fc = era5GenericAnomalyToSquares([
      { latitude: 45, longitude: 5, anomaly: -42.7 },
    ])
    expect(fc.features[0].properties).toEqual({ anomaly: -42.7 })
  })

  it('filters out points with anomaly === null', () => {
    const fc = era5GenericAnomalyToSquares([
      { latitude: 48, longitude: 2, anomaly: 10.0 },
      { latitude: 49, longitude: 3, anomaly: null },
    ])
    expect(fc.features).toHaveLength(1)
    expect(fc.features[0].properties.anomaly).toBe(10.0)
  })
})
