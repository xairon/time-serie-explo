...
export default function DataPage() {
  ...
  const tabs: { key: Tab; label: string }[] = [
    { key: 'import', label: t('mainPages.data.import') },
    { key: 'explore', label: dsCount > 0 ? t('mainPages.data.exploreCount', { count: dsCount }) : t('mainPages.data.explore') },
  ]
    ...
    updateMutation.mutate(
      {
        ...
        preprocessing: { fill_method: fillMethod, normalize },
      },
      {
        onSuccess: () => {
          setConfigDirty(false)
  ...
  const handleDeleteDataset = (id: string) => {
    ...
    if (!confirm(t('mainPages.data.deleteConfirm', { name: ds?.name ?? id }))) return
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-text-primary mb-1">{t('mainPages.data.title')}</h1>
        <p className="text-sm text-text-secondary">
          {t('mainPages.data.subtitle')}
        </p>
      </div>
        ...
        <div className="space-y-6">
          ...
          {isLoading ? (
            ...
            <div className="bg-bg-card rounded-xl border border-white/5 p-12 text-center">
              <p className="text-sm text-text-muted">{t('mainPages.data.noDatasets')}</p>
            </div>
          )}
          ...
          {selectedDataset && (
            <>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="bg-bg-card rounded-xl border border-white/5 p-4 text-center">
                      <p className="text-[10px] text-text-secondary uppercase tracking-wide">{t('mainPages.data.rows')}</p>
                      <p className="text-2xl font-bold text-text-primary">{qualityStats.totalRows.toLocaleString()}</p>
                    </div>
                    <div className="bg-bg-card rounded-xl border border-white/5 p-4 text-center">
                      <p className="text-[10px] text-text-secondary uppercase tracking-wide">{t('mainPages.data.columns')}</p>
                  ...
                  <div className="bg-bg-card rounded-xl border border-white/5 p-4">
                    <h3 className="text-sm font-semibold text-text-primary mb-3">{t('mainPages.data.missingPerColumn')}</h3>
                    <div className="h-[300px]">
                      <Plot
                        ...
                        layout={{
                          ...darkLayout,
                          xaxis: { ...darkLayout.xaxis, title: { text: t('mainPages.data.axisColumn') }, tickangle: -45 },
                          yaxis: { ...darkLayout.yaxis, title: { text: t('mainPages.data.missingValuesAxis') } },
                  ...
                  <div className="bg-bg-card rounded-xl border border-white/5 p-4">
                    <h3 className="text-sm font-semibold text-text-primary mb-3">{t('mainPages.data.completenessPerColumn')}</h3>
                    <div className="overflow-x-auto rounded-lg border border-white/5">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-bg-hover">
                            <th className="px-3 py-2 text-left text-xs font-medium text-text-secondary uppercase">{t('mainPages.data.column')}</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-text-secondary uppercase">{t('mainPages.data.type')}</th>
                            <th className="px-3 py-2 text-right text-xs font-medium text-text-secondary uppercase">{t('mainPages.data.missing')}</th>
                            <th className="px-3 py-2 text-right text-xs font-medium text-text-secondary uppercase">{t('mainPages.data.completeness')}</th>
              ...
              {exploreSubTab === 'config' && (
                <div className="max-w-2xl space-y-6">
                  {datasetVariables.length > 0 ? (
                    <>
                      <div className="bg-bg-card rounded-xl border border-white/5 p-5 space-y-4">
                        <h3 className="text-sm font-semibold text-text-primary">{t('mainPages.data.targetVariable')}</h3>
                        <select
                      ...
                      <div className="bg-bg-card rounded-xl border border-white/5 p-5 space-y-4">
                        <h3 className="text-sm font-semibold text-text-primary">{t('mainPages.data.covariates')}</h3>
                      ...
                      <div className="bg-bg-card rounded-xl border border-white/5 p-5 space-y-4">
                        <h3 className="text-sm font-semibold text-text-primary">{t('mainPages.data.preprocessing')}</h3>
                        <div>
                          <label className="block text-xs text-text-secondary mb-1">
                            {t('mainPages.data.fillMethodLabel')}
                          </label>
                          <select
                            value={fillMethod}
                            ...
                            className="w-full bg-bg-input text-text-primary border border-white/10 rounded-lg px-3 py-2 text-sm"
                          >
                            <option value="interpolate">{t('mainPages.data.fillInterpolate')}</option>
                            <option value="ffill">{t('mainPages.data.fillFfill')}</option>
                            <option value="bfill">{t('mainPages.data.fillBfill')}</option>
                            <option value="drop">{t('mainPages.data.fillDrop')}</option>
                          </select>
                        </div>
                        <div className="flex items-center gap-3">
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input
                              type="checkbox"
                              checked={normalize}
                              onChange={(e) => { setNormalize(e.target.checked); setConfigDirty(true) }}
                          ...
                          <span className="text-xs text-text-secondary">
                            {t('mainPages.data.normalization')}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        >
                          {updateMutation.isPending ? t('mainPages.data.savingConfig') : t('mainPages.data.saveConfig')}
                        </button>
                        {saveSuccess && (
                          <span className="text-xs text-accent-green">{t('mainPages.data.configSaved')}</span>
                        )}
                        {updateMutation.isError && (
                          <span className="text-xs text-accent-red">
                            {t('mainPages.data.errorPrefix')} {updateMutation.error instanceof Error ? updateMutation.error.message : t('mainPages.data.failed')}
                          </span>
                        )}
                      </div>
                    </>
