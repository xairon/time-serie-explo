...
const RESOLUTION_OPTIONS: { value: Resolution; label: string }[] = [{ value: 'daily', label: 'Journalier' }, { value: 'monthly', label: 'Mensuel' }, { value: 'yearly', label: 'Annuel' }]
...
function formatDuration(months: number | null | undefined): string { if (!months) return '--'; const years = Math.floor(months / 12); const rem = months % 12; if (years === 0) return `${rem} mois`; if (rem === 0) return `${years} ans`; return `${years} ans ${rem} mois` }
function formatPeriod(start: string | null | undefined, end: string | null | undefined): string { if (!start && !end) return '--'; const fmt = (d: string) => new Date(d).toLocaleDateString('fr-FR', { year: 'numeric', month: 'short' }); if (start && end) return `${fmt(start)} -- ${fmt(end)}`; if (end) return `jusqu'à ${fmt(end)}`; return `depuis ${fmt(start!)}` }
...
export default function StationPage() {
  ...
  if (stationError) return (<div className="flex flex-col items-center justify-center h-full gap-4"><p className="text-red-400">Impossible de charger cette station — l'API est peut-être inaccessible.</p><button onClick={() => window.location.reload()} className="px-3 py-1.5 text-xs rounded-lg bg-accent-cyan/10 text-accent-cyan hover:bg-accent-cyan/20">Réessayer</button><Link to="/" className="text-accent-cyan hover:underline text-sm">Retour à l'observatoire</Link></div>)
  if (!station) return (<div className="flex flex-col items-center justify-center h-full gap-4"><p className="text-text-secondary">Station introuvable</p><Link to="/" className="text-accent-cyan hover:underline text-sm">Retour à l'observatoire</Link></div>)
  ...
  const valueLabel = isPiezo ? 'Niveau d\'eau (m NGF)' : `${hydroLabel} (${hydroUnit})`
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        <div className="flex items-center gap-4">
          <Link to="/" className="p-2 hover:bg-bg-hover rounded-lg transition-colors" aria-label="Retour à l'observatoire"><ArrowLeft className="w-5 h-5 text-text-secondary" /></Link>
          <div className="flex-1">
            <p className="text-xs text-accent-cyan font-medium uppercase tracking-wide">{isPiezo ? 'Piézométrie' : 'Hydrométrie'}</p>
        ...
        <section className="bg-gray-900/50 rounded-xl border border-white/5 p-4">
          <h2 className="text-sm font-semibold text-gray-300 mb-3 flex items-center gap-2"><Info className="w-4 h-4" />Fiche technique</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6">
            {isPiezo ? (<><div><MetaRow label="Période des données" value={formatPeriod(station.premiere_mesure, station.derniere_mesure)} /><MetaRow label="Durée" value={formatDuration(station.nb_mois_total)} /><MetaRow label="Mesures" value={station.nb_mesures_total?.toLocaleString() ?? null} /><MetaRow label="Dernière mesure" value={formatDateEN(station.derniere_mesure)} /><MetaRow label="Altitude station" value={station.altitude_station != null ? `${station.altitude_station.toFixed(0)} m NGF` : null} /></div><div><MetaRow label="Code BSS" value={station.code_bss ?? null} mono /><MetaRow label="Coordonnées" value={station.latitude != null && station.longitude != null ? `${station.latitude.toFixed(4)} N, ${station.longitude.toFixed(4)} E` : null} /><MetaRow label="Code BDLISA" value={station.codes_bdlisa ? (<a href={`https://bdlisa.eaufrance.fr/hydrogeounit/${station.codes_bdlisa.split(',')[0]}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">{station.codes_bdlisa}</a>) : null} /><MetaRow label="Min historique" value={station.niveau_min_absolu != null ? `${station.niveau_min_absolu.toFixed(2)} m NGF` : null} /><MetaRow label="Max historique" value={station.niveau_max_absolu != null ? `${station.niveau_max_absolu.toFixed(2)} m NGF` : null} /></div><div className="col-span-full flex flex-wrap gap-3 pt-2 border-t border-white/5"><a href={`https://ades.eaufrance.fr/Fiche/PtEau?code=${station.code_bss}`} target="_blank" rel="noopener noreferrer" className="text-[11px] text-blue-400 hover:underline">ADES</a><a href="https://hubeau.eaufrance.fr/page/api-piezometrie" target="_blank" rel="noopener noreferrer" className="text-[11px] text-blue-400 hover:underline">Hub'Eau</a></div></>) : (<><div><MetaRow label="Période des données" value={formatPeriod(station.premiere_mesure, station.derniere_mesure)} /><MetaRow label="Durée" value={formatDuration(station.nb_mois_total)} /><MetaRow label="Dernière mesure" value={formatDateEN(station.derniere_mesure)} /></div><div><MetaRow label="Code station" value={station.code_station ?? null} mono /><MetaRow label="Coordonnées" value={station.latitude_station != null && station.longitude_station != null ? `${station.latitude_station.toFixed(4)} N, ${station.longitude_station.toFixed(4)} E` : null} /><MetaRow label="Code cours d'eau" value={station.code_cours_eau ? (<a href={`https://services.sandre.eaufrance.fr/Courdo/Fiche/client/fiche_courdo.php?CdSandre=${station.code_cours_eau}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline font-mono">{station.code_cours_eau}</a>) : null} /><MetaRow label="Min historique" value={station.resultat_min_global != null ? `${station.resultat_min_global.toFixed(2)} ${hydroUnit}` : null} /><MetaRow label="Max historique" value={station.resultat_max_global != null ? `${station.resultat_max_global.toFixed(2)} ${hydroUnit}` : null} /></div><div className="col-span-full flex flex-wrap gap-3 pt-2 border-t border-white/5"><a href={`https://www.vigicrues.gouv.fr/niv3-station.php?CdStationHydro=${station.code_station}`} target="_blank" rel="noopener noreferrer" className="text-[11px] text-blue-400 hover:underline">VigiCrues</a><a href="https://hubeau.eaufrance.fr/page/api-hydrometrie" target="_blank" rel="noopener noreferrer" className="text-[11px] text-blue-400 hover:underline">Hub'Eau</a></div></>)}
          </div>
        </section>

        {!isPiezo && hydroSiblings && hydroSiblings.siblings.length > 0 && (
          <section className="bg-gray-900/50 rounded-xl border border-white/5 p-4">
            <h2 className="text-sm font-semibold text-gray-300 mb-1 flex items-center gap-2"><Waves className="w-4 h-4" />Site hydrométrique - {hydroSiblings.nb_stations} stations</h2>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div role="group" aria-label="Résolution temporelle" className="flex gap-1">{RESOLUTION_OPTIONS.filter(opt => { if (opt.value === 'monthly') { const loading = isPiezo ? piezoMonthlyLoading : hydroMonthlyLoading; if (loading) return true; return (monthly?.length ?? 0) > 0 }; return true }).map(opt => (<button key={opt.value} aria-pressed={resolution === opt.value} onClick={() => setResolution(opt.value)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${resolution === opt.value ? 'bg-accent-cyan/20 text-accent-cyan' : 'text-text-secondary hover:text-text-primary'}`}>{opt.label}</button>))}</div>
          {resolution === 'daily' && (<div className="flex items-center gap-2 ml-2"><input type="date" aria-label="Date de début" value={dailyStart ?? ''} onChange={(e) => setDailyStart(e.target.value || undefined)} className="bg-bg-card border border-white/10 rounded-lg px-2 py-1 text-xs text-text-primary focus:outline-none focus:border-accent-cyan/50" /><span className="text-xs text-text-secondary">-</span><input type="date" aria-label="Date de fin" value={dailyEnd} onChange={(e) => setDailyEnd(e.target.value)} className="bg-bg-card border border-white/10 rounded-lg px-2 py-1 text-xs text-text-primary focus:outline-none focus:border-accent-cyan/50" /></div>)}
        </div>

        {activeLoading ? <SkeletonChart /> : activeData && activeData.length > 0 ? (<div className="bg-bg-card border border-white/5 rounded-xl p-5"><TimeseriesChart data={activeData} valueKey={valueKey} valueLabel={valueLabel} unit={unit} precipKey={resolution === 'yearly' ? 'precipitation_totale_annuelle' : 'precipitation_totale'} percentiles={undefined} resolution={resolution} defaultPeriod={resolution === 'daily' ? 60 : Infinity} onPeriodChange={resolution === 'daily' ? handleDailyPeriodChange : undefined} /></div>) : (<div className="bg-bg-card border border-white/5 rounded-xl p-5 flex items-center justify-center h-64 text-text-secondary text-sm">Aucune donnée pour cette résolution</div>)}

        {(droughtData && droughtData.length > 0) || (spiData && spiData.length > 0) ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {droughtData && droughtData.length > 0 && (<div className="bg-bg-card border border-white/5 rounded-xl p-5"><DroughtIndexChart data={droughtData} indexKey={isPiezo ? 'spli' : 'ssfi'} label={isPiezo ? 'Indice Piézométrique Standardisé (SPLI/IPS)' : 'Indice Standardisé de Débit (SSFI)'} /></div>)}
            {spiData && spiData.length > 0 && (<div className="bg-bg-card border border-white/5 rounded-xl p-5"><DroughtIndexChart data={spiData} indexKey="spi" label="Indice Standardisé de Précipitation (SPI)" /></div>)}
          </div>
        ) : null}
