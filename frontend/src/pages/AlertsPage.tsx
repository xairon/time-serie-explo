...
const SEVERITY_TABS = [
  { key: 'EXTREMEMENT_BAS', label: 'Extr. bas', description: 'Niveau extrêmement bas (< -1,75s) — situation exceptionnelle' },
  { key: 'TRES_BAS', label: 'Très bas', description: 'Niveau très bas (-1,75 à -1,28s) — intervention possible' },
  { key: 'BAS', label: 'Bas', description: 'Niveau bas (-1,28 à -0,84s) — surveillance' },
  { key: 'HAUT', label: 'Haut', description: 'Niveau haut (0,84 à 1,28s) — au-dessus de la normale' },
  { key: 'TRES_HAUT', label: 'Très haut', description: 'Niveau très haut (1,28 à 1,75s) — risque d\'inondation' },
  { key: 'EXTREMEMENT_HAUT', label: 'Extr. haut', description: 'Niveau extrêmement haut (> 1,75s) — situation exceptionnelle' },
] as const
...
export default function AlertsPage() {
  ...
  const exportCSV = () => {
    ...
    const header = ['Code', 'Nom', 'Département', 'Classification', 'Dernière mesure', 'Depuis', 'Années consécutives', 'Type'].map(escapeCSV).join(',') + '\n'
  ...
  if (isError) return (<div className="h-full flex items-center justify-center"><p className="text-red-400 text-sm" role="alert">Erreur lors du chargement des données.</p></div>)
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-xl font-bold text-text-primary">Alertes</h1><p className="text-xs text-text-secondary">Stations actives (mesures &lt; 90 jours) en conditions anormales</p></div>
            {!isLoading && <span className="text-sm font-mono text-accent-cyan bg-accent-cyan/10 px-2 py-0.5 rounded-full">{totalAlerts}</span>}
          </div>
          <button onClick={exportCSV} aria-label="Exporter CSV" className="flex items-center gap-1.5 px-3 py-1.5 bg-bg-card border border-white/10 rounded-lg text-xs text-text-secondary hover:text-text-primary transition-colors"><Download className="w-3.5 h-3.5" /> CSV</button>
        </div>
