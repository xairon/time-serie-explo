...
async def get_dataset(dataset_id: str):
    ...
    return DatasetDetail(
        ...
        station_column=ds.station_column,
        stations=ds.stations,
        covariate_columns=ds.covariate_columns,
...
async def import_from_db(
    ...
    if req.date_column and req.date_column in df.columns:
        df[req.date_column] = __import__("pandas").to_datetime(df[req.date_column])
        df = df.set_index(req.date_column).sort_index()

    target_col = req.columns[0] if req.columns else ""
    cov_cols = req.columns[1:] if len(req.columns) > 1 else []
