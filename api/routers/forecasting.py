...
async def single_forecast(req: ForecastRequest, current: User = Depends(get_current_user)):
    ...
    # Per-window enrichment: only direction accuracy is meaningful on a single short
    # horizon. Variance-based scores (NSE/KGE/NRMSE) and ratio scores (WAPE) explode or
    # mislead when the observed window barely moves; they are reported at MODEL level
    # (full-test rolling) via the model record instead. Window panel stays interpretable:
    # MAE / RMSE / bias / sMAPE (physical units) + Dir_Acc.
    def _enrich_metrics(metrics: dict, pred_ts, target_ts) -> dict:
        ...
        try:
            ...
            if min_len > 1:
                actual_dir = np.diff(tgt_vals[:min_len]) > 0
                pred_dir = np.diff(pred_vals[:min_len]) > 0
